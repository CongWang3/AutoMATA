...
def prepare_data(seed, batch_size, ratio="8:1:1", kfold=0):
    ...
    if ratio == "0" and kfold == 0:
        ...
        data = pd.read_csv(PATH_TEST, sep="\t")
        data = data.dropna().values.astype(float) 
        data = scaler.transform(data)
        data = torch.FloatTensor(data)
        ...
        # data = pd.read_csv("D:\\wamp\www\\multi_omics_own\\download_data\\Jobs\\"+jobid+"\\"+jobid+"_data.txt",sep="\t")   # the path of training dataset
        data = pd.read_csv(PATH_DATA, sep="\t")   # the path of training dataset
        data = data.dropna().values.astype(float)
        data = scaler.fit_transform(data)
        data = torch.FloatTensor(data)
        train_loader = DataLoader(TensorDataset(data), batch_size=batch_size, shuffle=True)
        val_loader = 0
        data = pd.read_csv(PATH_TEST, sep="\t")   # the path of testing dataset
        data = data.dropna().values.astype(float)
        data = scaler.transform(data)
        data = torch.FloatTensor(data)
        test_loader = DataLoader(TensorDataset(data), batch_size=batch_size, shuffle=True)
...
def kfold_cross_validation(args, k_folds=5):
    ...
    dim, train_loader, val_loader, test_loader, scaler = prepare_data(
        args.random_seed, args.batch_size, args.ratio, k_folds
    )
    train_data_list = []
...
def main():
    ...
    parser.add_argument('--loss_function', type=str, default='mse', 
                       ...
                       help='reconstruction loss function type:\n'
                            'basic loss: mse(mean squared error), bce(binary cross entropy), mae/l1(mean absolute error), smooth_l1(smooth L1), huber(Huber loss)\n'
                            'advanced loss: focal(Focal loss), contrastive(contrastive loss), spectral(spectral loss), wasserstein(Wasserstein distance)\n'
                            'similarity loss: perceptual(perceptual loss), cosine(cosine similarity), kl_div(KL divergence), infonce(InfoNCE)\n'
                            'regularization: regularization(regularization loss)')
    ...
    parser.add_argument('--scaler_path', type=str, default='scaler.pkl', help='scaler save path')
    parser.add_argument('--evaluate_model', action='store_true', default=1, help='evaluate model')
    parser.add_argument('--save_evaluation', action='store_true', default=1, help='save evaluation')
    parser.add_argument('--evaluation_path', type=str, default='results.png', help='evaluation save path')
    ...
    if args.k_folds:
        ...
        print("=" * 50)
        dim, train_loader, val_loader, test_loader, scaler = prepare_data(
            args.random_seed, args.batch_size, args.ratio, args.k_folds
        )
        model = VAE(
            input_dim=dim,
            hidden_dims=[512, 256, 128],
