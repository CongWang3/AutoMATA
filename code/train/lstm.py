...
def train(dataloader, model):
    ...
    train_precision, train_recall, train_f1 = precision_recall_fscore_support(
...
def validate(dataloader, model):
    ...
    val_precision, val_recall, val_f1 = precision_recall_fscore_support(
        y_true, y_pred, average='weighted'
    )[:-1]
    ...
    def __init__(
        ...
        if loss_function == "crossentropy":
            ...
            self.criterion = FocalLoss(
                gamma=2, alpha=0.25, task_type='multi-class', num_classes=output_size
            )
        elif loss_function == "nllloss":
            self.criterion = nn.NLLLoss()
        self.optimier = create_optimizer_with_reg(
            self, optimizer_function, lr, r_method, r_weight
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--optimizer_function", default="adam", type=str)
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Regularization method: l1, l2, maxnorm, sparsity, or none',
    )
    parser.add_argument('--r_weight', type=float, default=0.0)
    parser.add_argument('--dropout_rate', type=float, default=0.0)
    parser.add_argument('--feature_method', type=str, default=None)
    ...
    feature_method = (
        args.feature_method if args.feature_method and args.feature_method.strip() else None
    )
    ...
    if ratio != "0":
        process(ratio=ratio)
    X_train, Y_train, feature_indices = load_data(
        "train", feature_method=feature_method
    )
    actual_num_classes = len(torch.unique(Y_train))
    if actual_num_classes != output_size:
        print(
            f"Warning: The number of classes set by the user ({output_size}) does not match "
            f"the actual number of classes in the data ({actual_num_classes})"
        )
    ...
    if kfold:
        ...
        for i, (train_idx, val_idx) in enumerate(skf.split(X_train, Y_train)):
            ...
            trainset = torch.FloatTensor(np.array(X_train)[train_idx])
            valset = torch.FloatTensor(np.array(X_train)[val_idx])
            traintag = torch.LongTensor(np.array(Y_train)[train_idx])
            valtag = torch.LongTensor(np.array(Y_train)[val_idx])
            ...
            lstm = LSTMModel(
                input_size, hidden_size, output_size, num_layers, dropout_rate, r_method, r_weight
            ).to(device)
            val_acc_s = []
            val_loss_s = []
            train_acc_s = []
            ...
            for t in range(epochs):
                ...
                train_loss, train_acc, train_precision, train_recall, train_f1 = train(
                    dataloader=train_loader, model=lstm
                )
                val_loss, val_acc, val_precision, val_recall, val_f1 = validate(
                    dataloader=val_loader, model=lstm
                )
                print(
                    "train loss = {}, acc = {}, precision = {}, recall = {}, f1 = {} ".format(
                        round(train_loss, 4),
                        round(train_acc, 4),
                ...
                print(
                    "validation loss = {}, acc = {}, precision = {}, recall = {}, f1 = {}".format(
                        round(val_loss, 4),
                        round(val_acc, 4),
            ...
            val_acc, val_precision, val_recall, val_f1 = test(
                dataloader=val_loader, model=lstm
            )
            print(
                "validation acc = {}, precision = {}, recall = {}, f1 = {}".format(
                    round(val_acc, 4),
                    round(val_precision, 4),
                    round(val_recall, 4),
                    round(val_f1, 4),
        ...
        kfscore = np.array(kfscore).sum(axis=0) / float(kfold)
        print("--------KFold Final Average Validation Results---------")
        print(
            "Stratified KFold mean validation acc = {}, precision = {}, recall = {}, f1 = {}".format(
                round(kfscore[0], 4),
                round(kfscore[1], 4),
                round(kfscore[2], 4),
                round(kfscore[3], 4),
